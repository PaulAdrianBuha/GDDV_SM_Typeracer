...
try {
    $db = new PDO('sqlite:../private/typeracer.db');
...
switch ($accio) {
    case 'join':
        $_SESSION['player_id'] = $_GET['user_id'];
    ...
    case 'init_status':
        ...
        $player_id = $_SESSION['player_id'];
        $stmt = $db->prepare('SELECT users1.user_id as user_id_1, users1.user_name as user_name_1, users2.user_id as user_id_2, users2.user_name as user_name_2
            FROM games LEFT JOIN users as users1 ON users1.user_id = games.player1 LEFT JOIN users as users2 ON users2.user_id = games.player2
            WHERE game_id = :game_id');
        $stmt->bindValue(':game_id', $game_id);
        $stmt->execute();
        $usersInfo = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($player_id == $usersInfo['user_id_1']) {
            $usernamePlayer = $usersInfo['user_name_1'];
            $usernameOtherPlayer = $usersInfo['user_name_2'];
        } else {
            $usernamePlayer = $usersInfo['user_name_2'];
            $usernameOtherPlayer = $usersInfo['user_name_1'];
        }
        echo json_encode(['usernamePlayer' => $usernamePlayer, 'usernameOtherPlayer' => $usernameOtherPlayer]);
        break;
